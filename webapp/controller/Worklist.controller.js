sap.ui.define([
	...
	return BaseController.extend("opensap.manageproducts.controller.Worklist", {
		...
		onDelete: function(oItem) {
			...
			var product = oItem.getSource().getParent();
			var path = product.getBindingContext().getPath();
			// var str = path.substring(path.lastIndexOf(" ") + 1);
			// var str1 = str.substring(0, (str.length - 2));
			// //var idx = parseInt(str1);

			// var str2 = path.substring(path.lastIndexOf('(') + 1);
			// var str3 = str2.substring(1, (str2.length - 2));
			///ProductSet('ProductID 10')
			var m = oTable.getModel();
			var d = m.getProperty(path);
			var pname = d.Name;
			m.remove(path, null, function() {
				MessageToast.show(pname + " deleted");
			}, function() {
				MessageToast("Delete failed");
			});
			...
			// var product = oItem.getSource().getParent();
			// var oPath = product.getBindingContext().getPath();
			// var oIndex = parseInt(oPath.substring(oPath.lastIndexOf('/') + 1));
			// var oTable = this.getView().byId("table");
			// var m = oTable.getModel();
			// var data = m.getProperty("/ProductSet");
			// var removed = data.splice(oIndex, 1);
			// m.setProperty("/modelData", data);

			//MessageToast.show(JSON.stringify("removed[0]" + 'is removed'));

			// var oTable = this.byId("table");
			// var oTable1 = this.getView().byId("table");
			// var m = oTable1.getModel();

			// var ok = 'ok';

			// MessageToast.show(oItem.getSource().getId() + " deleted");
